...
This crate is a pure-Rust, host-testable library (`cargo test -p difftest`). The RISC Zero guest `main.rs` wiring — `env::read()` of manifest inputs, `check()` call, `env::commit()` of the `Journal` — follows the `template` crate once `feat/risc0-toolchain-matmul` merges. The `Journal` struct in `src/journal.rs` is a local copy of the toon-meta#121 envelope, to be replaced by the shared `journal` crate (see `TODO(dedup)` marker).
